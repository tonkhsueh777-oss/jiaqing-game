...
function selectedGuide(action, selected, game, state) {
  if (!selected) return '请选择一张手牌。右侧会显示这张牌能做什么，以及下一步要选择的目标。';
  if (!action) return game.getCardUnavailableReason?.(state, 'human', selected.card) || '这张牌目前不能使用。';
  ...
  if (action.type === 'trump') return '下一步选择一名拥有宝物的对手，再按照王牌交换流程选择双方宝物。';
...
export function buildHudModel(state, game, selectedRuntimeId = null) {
  ...
  return {
    ...
    selected,
    selectedAction,
    guide: selectedGuide(selectedAction, selected, game, state)
  };
}
